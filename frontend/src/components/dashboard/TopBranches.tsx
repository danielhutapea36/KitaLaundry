                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    ...
                    <div className="flex items-center space-x-1 text-gray-600">
                      ...
                      <span>Rp{branch.totalRevenue.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center space-x-1 text-gray-600">
                      <TrendingUp className="w-3 h-3" />
                      <span>Rp{Math.round(branch.avgOrderValue)}/order</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {safeBranches.length > 0 && (
        <div className="mt-6 pt-4 border-t border-gray-200">
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900">
                Rp{safeBranches.reduce((sum, branch) => sum + (branch.totalRevenue || 0), 0).toLocaleString()}
              </div>
              <div className="text-xs text-gray-500">Total Revenue</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-gray-900">
                Rp{Math.round(safeBranches.reduce((sum, branch) => sum + (branch.avgOrderValue || 0), 0) / (safeBranches.length || 1)).toLocaleString()}
              </div>
