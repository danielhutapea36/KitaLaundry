...
import { PRICING_FAQ_DATA } from '@/data/dummyData'
              ...
              {pricingData[activeCategory]?.length > 0 ? (
                pricingData[activeCategory].map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-center text-gray-600">{item.washFold > 0 ? `Rp${item.washFold}` : '-'}</td>
                    <td className="py-4 px-4 text-center text-gray-600">{item.washIron > 0 ? `Rp${item.washIron}` : '-'}</td>
                    <td className="py-4 px-4 text-center text-gray-600">{item.dryClean > 0 ? `Rp${item.dryClean}` : '-'}</td>
                    <td className="py-4 px-4 text-center text-gray-600">{item.steamPress > 0 ? `Rp${item.steamPress}` : '-'}</td>
                  </tr>
                ))
              ) : (
                <tr>
...
export default function PricingPage() {
  ...
  const faqData = PRICING_FAQ_DATA
