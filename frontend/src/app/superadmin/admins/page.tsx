...
import { PresetRoleSelector } from '@/components/rbac/PresetRoleSelector'
...
export default function AdminsPage() {
  ...
  const [editCenterAdminPermissions, setEditCenterAdminPermissions] = useState(getDefaultCenterAdminPermissions())
  ...
  const openEditModal = async (admin: Admin) => {
    try {
      ...
      if (d.data?.admin?.role === 'center_admin') {
        setEditCenterAdminPermissions(d.data?.admin?.permissions || getDefaultCenterAdminPermissions())
      } else {
        setEditPermissions(d.data?.admin?.permissions || getDefaultPermissions())
      }
  ...
  const handleUpdatePermissions = async () => {
    ...
    setUpdating(true)
    try {
      const permsToSave = selectedAdmin.role === 'center_admin' ? editCenterAdminPermissions : editPermissions
      await apiCall(`/superadmin/admins/${selectedAdmin._id}/permissions`, { method: 'PUT', body: JSON.stringify({ permissions: permsToSave }) })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl p-4">
          ...
          <div className="text-sm text-green-100">Center Admins</div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="px-4 py-2 border rounded-lg">
            ...
            <option value="center_admin">Center Admin Only</option>
          </select>
        </div>
      </div>
      )}
              ...
              <tbody className="divide-y divide-gray-200">
                {paginatedAdmins.map((admin) => (
                  <tr key={admin._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${roleColors[admin.role] || 'bg-gray-100 text-gray-800'}`}>
                        {admin.role === 'center_admin' ? 'Center Admin' : 'Admin'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
          ...
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-2 mb-4">
                ...
                <button onClick={() => setCreateRole('center_admin')} className={`flex-1 py-2 px-4 rounded-lg border ${createRole === 'center_admin' ? 'bg-green-500 text-white border-green-500' : 'border-gray-300'}`}>
                ...
                <div>
                  ...
                  {showPermissions && (
                    <div className="mt-4 border rounded-lg p-4">
                      <PresetRoleSelector onSelect={(p) => setNewAdminPermissions(p)} />
                      <div className="mt-4"><PermissionMatrix permissions={newAdminPermissions} onChange={setNewAdminPermissions} /></div>
                    </div>
                  )}
                </div>
              )}
              {createRole === 'center_admin' && (
                <div>
                  <button onClick={() => setShowPermissions(!showPermissions)} className="flex items-center gap-2 text-sm font-medium text-green-600">
                    {showPermissions ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    {showPermissions ? 'Hide' : 'Configure'} Center Admin Permissions
                  </button>
                  {showPermissions && (
                    <div className="mt-4 border rounded-lg p-4">
                      <div className="flex gap-2 mb-4">
                        <button type="button" onClick={() => setNewCenterAdminPermissions(getFullCenterAdminPermissions())} className="px-3 py-1 text-sm bg-green-100 text-green-700 rounded-lg hover:bg-green-200">
                          Full Access
                        </button>
                        <button type="button" onClick={() => setNewCenterAdminPermissions(getDefaultCenterAdminPermissions())} className="px-3 py-1 text-sm bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
                          Clear All
                        </button>
                      </div>
                      <CenterAdminPermissionMatrix permissions={newCenterAdminPermissions} onChange={setNewCenterAdminPermissions} />
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="bg-white rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b">
              <div>
                ...
                <span className={`inline-block mt-1 px-2 py-0.5 text-xs font-medium rounded-full ${roleColors[selectedAdmin.role] || 'bg-gray-100 text-gray-800'}`}>
                  {selectedAdmin.role === 'center_admin' ? 'Center Admin' : 'Admin'}
                </span>
              </div>
              <button onClick={() => setShowEditModal(false)} className="p-2 hover:bg-gray-100 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6">
              {selectedAdmin.role === 'center_admin' ? (
                <>
                  <div className="flex gap-2 mb-4">
                    <button type="button" onClick={() => setEditCenterAdminPermissions(getFullCenterAdminPermissions())} className="px-3 py-1 text-sm bg-green-100 text-green-700 rounded-lg hover:bg-green-200">
                      Full Access
                    </button>
                    <button type="button" onClick={() => setEditCenterAdminPermissions(getDefaultCenterAdminPermissions())} className="px-3 py-1 text-sm bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
                      Clear All
                    </button>
                  </div>
                  <CenterAdminPermissionMatrix permissions={editCenterAdminPermissions} onChange={setEditCenterAdminPermissions} />
                </>
              ) : (
                <>
                  <PresetRoleSelector onSelect={(p) => setEditPermissions(p)} />
                  <div className="mt-4"><PermissionMatrix permissions={editPermissions} onChange={setEditPermissions} /></div>
                </>
              )}
            </div>
            <div className="flex justify-end gap-3 p-6 border-t">
              <Button variant="outline" onClick={() => setShowEditModal(false)}>Cancel</Button>
              <Button onClick={handleUpdatePermissions} disabled={updating} className={selectedAdmin.role === 'center_admin' ? 'bg-green-500 hover:bg-green-600' : 'bg-purple-500 hover:bg-purple-600'}>
