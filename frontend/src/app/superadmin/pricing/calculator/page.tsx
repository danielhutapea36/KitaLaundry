                  ...
                  >
                    ...
                    {serviceItems.map((serviceItem) => (
                      <option key={serviceItem.name} value={serviceItem.name}>
                        {serviceItem.name} - Rp{serviceItem.basePrice} ({serviceItem.unit})
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {calculation.calculation.itemDetails.map((item: any, index: number) => (
                    <div key={index} className="flex justify-between text-sm">
                      ...
                      <span>Rp{item.totalPrice.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="border-t pt-4 space-y-2">
                <div className="flex justify-between">
                  <span>Subtotal:</span>
                  <span>Rp{calculation.calculation.subtotal.toFixed(2)}</span>
                </div>
                ...
                {calculation.calculation.discount > 0 && (
                  <div className="flex justify-between text-green-600">
                    ...
                    <span>-Rp{calculation.calculation.discount.toFixed(2)}</span>
                  </div>
                )}
                
                <div className="flex justify-between">
                  <span>Tax ({calculation.calculation.taxAmount > 0 ? '18%' : '0%'}):</span>
                  <span>Rp{calculation.calculation.taxAmount.toFixed(2)}</span>
                </div>
                
                {calculation.calculation.deliveryCharge > 0 && (
                  <div className="flex justify-between">
                    <span>Delivery Charge:</span>
                    <span>Rp{calculation.calculation.deliveryCharge.toFixed(2)}</span>
                  </div>
                )}
                
                <div className="border-t pt-2 flex justify-between font-semibold text-lg">
                  <span>Total:</span>
                  <span>Rp{calculation.calculation.total.toFixed(2)}</span>
                </div>
              </div>
