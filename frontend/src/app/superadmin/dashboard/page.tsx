...
export default function SuperAdminDashboard() {
  ...
  const handleExport = () => {
    ...
    if (dashboardData.overview) {
      csvRows.push('Total Revenue (Rp),' + (dashboardData.overview.totalRevenue || 0))
      ...
      if (dashboardData.overview.totalOrders > 0) {
        ...
        csvRows.push('Average Order Value (Rp),' + avgOrderValue)
      }
    }
    csvRows.push('')
    csvRows.push('')
    ...
    if (dashboardData.topBranches && dashboardData.topBranches.length > 0) {
      ...
      csvRows.push('')
      csvRows.push('Rank,Branch Name,Branch Code,Total Orders,Revenue (Rp)')
    ...
    if (dashboardData.revenue && dashboardData.revenue.length > 0) {
      ...
      csvRows.push('')
      csvRows.push('Date,Revenue (Rp),Number of Orders')
    ...
    if (dashboardData.recentOrders && dashboardData.recentOrders.length > 0) {
      ...
      csvRows.push('')
      csvRows.push('Order ID,Customer Name,Order Status,Amount (Rp),Order Date')
            ...
            {dashboardData?.topBranches && dashboardData.topBranches.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dashboardData.topBranches.map((branch: any) => ({
                  ...
                  <YAxis stroke="#9ca3af" fontSize={10} tickFormatter={(v) => `Rp${(v/1000).toFixed(0)}k`} />
                  <Tooltip 
                    contentStyle={{
                      backgroundColor: '#fff',
                      border: '1px solid #e5e7eb',
