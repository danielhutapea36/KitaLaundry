...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    ...
    try { 
      ...
      // Filter out admin and center_admin roles - only show regular users
      const allUsers = d.data?.users || d.data || []
      const filteredUsers = allUsers.filter((u: User) => !['admin', 'center_admin'].includes(u.role))
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">Manage staff, support agents and other users</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-orange-500 to-amber-600 rounded-xl p-4">
          <div className="text-2xl font-bold text-white">{users.filter(u => u.role === 'support_agent').length}</div>
          <div className="text-sm text-orange-100">Support Agents</div>
        </div>
